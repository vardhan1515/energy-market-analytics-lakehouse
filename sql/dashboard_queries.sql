...
-- Replace `workspace` if a different catalog widget was used.
...
SELECT
  ...
  actual_load_mw,
  forecast_load_mw
FROM workspace.gold.fact_energy_load
...
FROM workspace.gold.daily_peak_demand
WHERE market_date BETWEEN :start_date AND :end_date
...
FROM workspace.gold.regional_forecast_performance
WHERE market_date BETWEEN :start_date AND :end_date
GROUP BY region
...
SELECT
  ...
  status,
  is_blocking
FROM workspace.ops.data_quality_results
...
FROM workspace.silver.actual_load_hourly;
...
-- FROM workspace.gold.fact_energy_load l
-- JOIN workspace.silver.weather_hourly w USING (interval_start_utc)
...
-- FROM workspace.silver.realtime_price
